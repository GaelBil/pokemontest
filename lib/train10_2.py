...
def network(features, target):
    ...
    with tf.name_scope('SecondLayer'):
        w2 = tf.Variable(tf.random.normal([10,2]), name='Weight2')
    ...
    with tf.name_scope('Cost'):
        cost = tf.nn.softmax_cross_entropy_with_logits_v2(labels=tf_targets,
                                                       logits=z2)
#    cost_sum = tf.summary.scalar(name='Cost', tensor=cost)
    ...
    with tf.Session() as sess:
        logdir = "/tmp/tensorflow/graph/train10_2/" + datetime.now().strftime("%Y%m%d-%H%M%S") + "/"
