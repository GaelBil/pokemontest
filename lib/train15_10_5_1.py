...
def network(features, target):
    ...
    with tf.name_scope('FirstLayer'):
        w1 = tf.Variable(tf.random.normal([20,15]), name='Weight1')
        w1_hist = tf.summary.histogram('w1', w1)
        b1 = tf.Variable(tf.zeros([15]), name='Bias1')
    ...
    with tf.name_scope('SecondLayer'):
        w2 = tf.Variable(tf.random.normal([15,10]), name='Weight2')
    ...
    with tf.name_scope('ThirdLayer'):
        w3 = tf.Variable(tf.random.normal([10,5]), name='Weight3')
    ...
    # Third part
    with tf.name_scope('FourthLayer'):
        ...
        with tf.name_scope('Preactivation4'):
            z4 = tf.matmul(a3, w4) + b4
        with tf.name_scope('Activation4'):
            py = tf.nn.sigmoid(z4)
    ...
    with tf.Session() as sess:
        logdir = "/tmp/tensorflow/graph/train15_10_5_1/" + datetime.now().strftime("%Y%m%d-%H%M%S") + "/"
